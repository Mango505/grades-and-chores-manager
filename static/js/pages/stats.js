 ...
 * Layout:
 *   [Banner]
 *   [st-grid-tall]  Fächer & Trends  |  Notenverlauf & Gesamtschnitt
 *   [st-grid]       Short cards (CSS columns – no row gaps)
 */
...
async function renderStats(container) {
  ...
  // ── TALL CARD 1: Fächer & Trends ─────────────────────────────────────────
  const fachTrendCard =
    '<div class="card">' +
      '<p class="st-head">F\u00e4cher & Trends</p>' +
      subjectTrends.map(t => {
        const arrow  = t.slope < -.05 ? "\u2191" : t.slope > .05 ? "\u2193" : "\u2192";
        const aColor = t.slope < -.05 ? "#1b7e4a" : t.slope > .05 ? "var(--md-sys-color-error)" : "var(--md-sys-color-on-surface-variant)";
        return '<div class="grade-row" style="gap:8px">' +
          '<span style="flex:1;font-size:14px">' + t.name + '</span>' +
          '<span style="color:' + aColor + ';font-weight:700;font-size:16px;width:20px;text-align:center">' + arrow + '</span>' +
          '<span style="font-size:12px;color:var(--md-sys-color-on-surface-variant);min-width:52px;text-align:right">\u00d8 ' + t.avg.toFixed(2) + '</span>' +
        '</div>';
      }).join("") +
    '</div>';

  // ── TALL CARD 2: Notenverlauf ─────────────────────────────────────────────
  const gradeChartCard = hasGradeChart
    ? '<div class="card">' +
        '<p class="st-head">Notenverlauf <span style="font-weight:400;text-transform:none;letter-spacing:0;font-size:11px">(Note 1 = oben)</span></p>' +
  ...
  const bestWorstFachCard =
    '<div class="card">' +
      '<p class="st-head">Bestes & Schlechtestes Fach</p>' +
      '<div class="grade-row"><span class="material-symbols-rounded" style="font-size:20px;color:var(--md-sys-color-primary)">emoji_events</span>' +
        ...
        '<span style="font-size:12px;color:var(--md-sys-color-on-surface-variant);margin-left:6px">(\u00d8 ' + (subjectTrends[0]?.avg.toFixed(2) ?? "\u2014") + ')</span>' +
      '</div>' +
      '<div class="grade-row"><span class="material-symbols-rounded" style="font-size:20px;color:var(--md-sys-color-error)">sentiment_dissatisfied</span>' +
        ...
        '<span style="font-size:12px;color:var(--md-sys-color-on-surface-variant);margin-left:6px">(\u00d8 ' + (subjectTrends.at(-1)?.avg.toFixed(2) ?? "\u2014") + ')</span>' +
      '</div>' +
    '</div>';

  const notenCard =
    '<div class="card">' +
      '<p class="st-head">Einzelnoten</p>' +
      '<div class="grade-row">' + gradeBadge(best.value) +
        '<span style="flex:1;font-size:13px;color:var(--md-sys-color-on-surface-variant)">Beste Note</span>' +
        '<span style="font-size:13px">' + best.subject + '</span>' +
      '</div>' +
      '<div class="grade-row">' + gradeBadge(worst.value) +
        '<span style="flex:1;font-size:13px;color:var(--md-sys-color-on-surface-variant)">Schlechteste</span>' +
        '<span style="font-size:13px">' + worst.subject + '</span>' +
      '</div>' +
      '<div style="font-size:12px;color:var(--md-sys-color-on-surface-variant);margin-top:8px">' + all.length + ' Noten gesamt</div>' +
    '</div>';
      ...
      '<div style="display:flex;align-items:flex-end;gap:6px;height:90px;padding-top:16px">' +
        Object.entries(dist).map(([g, c]) =>
          '<div style="display:flex;flex-direction:column;align-items:center;gap:3px;flex:1">' +
            ...
            '<div style="width:100%;border-radius:3px 3px 0 0;min-height:3px;height:' + Math.round((c/mx)*60) + 'px;' +
              'background:' + (+g<=2?"var(--md-sys-color-primary)":+g<=4?"var(--md-sys-color-secondary)":"var(--md-sys-color-error)") + '"></div>' +
            '<span style="font-size:12px;font-weight:500">' + g + '</span>' +
          '</div>'
        ).join("") +
      '</div>' +
    '</div>';
  ...
  const tallSection = hasGradeChart
    ...
    : '<div style="margin-bottom:16px">' + fachTrendCard + '</div>';
...
// ---------------------------------------------------------------------------
// Exportieren – .btn-filled replaces md-filled-button
// ---------------------------------------------------------------------------
async function renderExport(container) {
  container.innerHTML =
    '<div class="card" style="max-width:480px">' +
      ...
      '</div>' +
      // .btn-filled: primary color, native button – reliable padding/margin
      '<button class="btn-filled" id="btnExport">' +
        '<span class="material-symbols-rounded">download</span>Als TXT herunterladen' +
      '</button>' +
    '</div>';
...
// ---------------------------------------------------------------------------
// Vergleichen – .btn-filled replaces md-filled-tonal-button (now primary color)
// ---------------------------------------------------------------------------
  ...
  container.innerHTML =
    '<div class="card" style="max-width:560px">' +
      ...
      '<div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:20px">' +
        '<div style="flex:1;min-width:160px"><p style="font-size:12px;margin-bottom:6px;color:var(--md-sys-color-on-surface-variant)">Export 1 (\u00e4lter)</p>' +
          '<input type="file" id="file1" accept=".txt" style="font-size:13px;width:100%"/></div>' +
        '<div style="flex:1;min-width:160px"><p style="font-size:12px;margin-bottom:6px;color:var(--md-sys-color-on-surface-variant)">Export 2 (neuer)</p>' +
          '<input type="file" id="file2" accept=".txt" style="font-size:13px;width:100%"/></div>' +
      '</div>' +
      // .btn-filled: primary color (was md-filled-tonal-button = secondary color)
      '<button class="btn-filled" id="btnCompare">Vergleichen</button>' +
    '</div>' +
...
function renderCmpResult(el, d1, d2) {
  const diff = (a, b, inv = false) => {
    ...
    const d = b - a, better = inv ? d < 0 : d > 0;
  ...
  const row = (label, v1, v2, d) =>
    '<tr>' + td('<span style="font-size:13px;color:var(--md-sys-color-on-surface-variant)">' + label + '</span>') + td(v1) + td(v2) + td(d) + '</tr>';
...
function buildExport(data, label, { incStats, incGrades, incLog, incRedeem }) {
  ...
  if (incStats && data.overall_average != null) {
    L.push("", "=== STATISTIKEN ===");
    L.push("Gesamtdurchschnitt: " + data.overall_average.toFixed(2), "Anzahl Noten: " + data.grade_count);
...
function parseExport(text) {
  ...
  for (const line of text.split("\n")) {
    if      (line.startsWith("Label:"))              r.label = line.split(":").slice(1).join(":").trim();
      ...
      const p = line.split("|"), name = p[0].replace("Fach:","").trim(), a = parseFloat((p[1]??"").replace("\u00d8",""));
      if (!isNaN(a)) r.subjects[name] = a;
    }
  }
  return r;
}
...
function ts() { return new Date().toISOString().slice(0,13).replace(/[-T:]/g,""); }
