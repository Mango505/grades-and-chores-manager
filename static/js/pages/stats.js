    ...
    <div class="st-grid">
      ...
      ${topL.length ? `<div class="card">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:8px">
          ${topL.map(([l,n])=>`<span style="padding:4px 12px;border-radius:var(--shape-corner-full);
            background:var(--md-sys-color-secondary-container);
            color:var(--md-sys-color-on-secondary-container);font-size:13px">${l} <strong>${n}×</strong></span>`).join("")}
        </div>
      </div>` : ""}
...
function renderCmpResult(el, d1, d2) {
  ...
  el.innerHTML = `<div class="card" style="overflow-x:auto">
    <table style="width:100%;border-collapse:collapse;font-size:13px">
      <thead><tr>
        <th style="text-align:left;padding:6px 8px;color:var(--md-sys-color-on-surface-variant);border-bottom:1px solid var(--md-sys-color-outline-variant)"></th>
        <th style="text-align:left;padding:6px 8px;color:var(--md-sys-color-on-surface-variant);border-bottom:1px solid var(--md-sys-color-outline-variant)">${d1.label}</th>
        <th style="text-align:left;padding:6px 8px;color:var(--md-sys-color-on-surface-variant);border-bottom:1px solid var(--md-sys-color-outline-variant)">${d2.label}</th>
        <th style="text-align:left;padding:6px 8px;color:var(--md-sys-color-on-surface-variant);border-bottom:1px solid var(--md-sys-color-outline-variant)">Δ</th>
      </tr></thead>
      <tbody>
...
function row(label, v1, v2, d) {
  ...
  return `<tr>${td(`<span style="font-size:13px;color:var(--md-sys-color-on-surface-variant)">${label}</span>`)}${td(v1)}${td(v2)}${td(d)}</tr>`;
}
...
function ts() {
  return new Date().toISOString().slice(0,16).replace(/[-T:]/g,"").replace("T","_").slice(0,13);
}
